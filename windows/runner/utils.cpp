...
std::string Utf8FromUtf16(const wchar_t* utf16_string) {
  ...
  // Bound the scan to the maximum UNICODE_STRING length to avoid reading past
  // an unterminated input buffer (CWE-126).
  constexpr size_t kUnicodeStringMaxChars = 32767;
  const int input_length = static_cast<int>(
      wcsnlen(utf16_string, kUnicodeStringMaxChars));
